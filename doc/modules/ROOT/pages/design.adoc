...
Scientific notation is the natural fit for cohort preservation because it mirrors the actual layout of the type, and it can represent every member of a cohort.
A second option, `cohort_preserving_fixed`, offers the same guarantee in fixed notation with an implied exponent of zero.
Because fixed notation has no exponent field, `to_chars` can only preserve members whose exponent is not positive; a positive exponent would require appending trailing zeros, which changes the cohort member that is recovered on read-back, so such values are rejected.
