...
.. The quantum operations `quantexp`, `samequantum`, and `quantize` are defined in terms of cohorts, so they are *rejected at compile time* for the fast types with a `static_assert` rather than silently returning a meaningless answer.
The exponent supplied at construction is not recoverable, because normalization makes `decimal_fast32_t{1, 0}` and `decimal_fast32_t{1000000, -6}` the same object.
Any quantum a fast type appears to carry reflects only the magnitude of its value, so these operations cannot do what their IEEE definitions promise.

Use one of the IEEE types if you need the quantum operations.
To round to a fixed number of fractional digits on any type, including the fast types, use xref:cmath.adoc#cmath_rescale[`rescale`] instead.
