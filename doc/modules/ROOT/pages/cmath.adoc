...
| https://en.cppreference.com/w/c/numeric/math/roundeven[roundeven] | Computes the nearest integer with halfway cases rounded to even
...
template <typename DecimalType>
constexpr DecimalType roundeven(DecimalType x) noexcept;
...
WARNING: This operation is defined in terms of cohorts, which the fast types do not have, so calling it with `decimal_fast32_t`, `decimal_fast64_t`, or `decimal_fast128_t` is a compile error.
See the xref:fast_types.adoc#fast_types_limitations[fast type limitations].
...
constexpr int quantexpd32(decimal32_t x) noexcept;
constexpr int quantexpd64(decimal64_t x) noexcept;
constexpr int quantexpd128(decimal128_t x) noexcept;
...
template <typename Decimal>
constexpr Decimal quantize(Decimal x, Decimal y) noexcept;
...
WARNING: This operation is defined in terms of cohorts, which the fast types do not have, so calling it with `decimal_fast32_t`, `decimal_fast64_t`, or `decimal_fast128_t` is a compile error.
Because those types normalize on construction, `y` cannot carry the exponent it was built with, so the operation cannot do what its definition promises.
See the xref:fast_types.adoc#fast_types_limitations[fast type limitations].
To round to a fixed number of fractional digits on any type, use xref:#cmath_rescale[`rescale`] instead.
...
This function decomposes the input decimal value into a struct containing the significand, exponent, and sign.
This function does not modify the stored value at all, so it returns the significand and exponent non-normalized i.e., in the cohort that they are stored in.
...
The function returns `val` rounded to `precision` digits after the decimal point, using the active rounding mode.
So `rescale(123.456789, 2)` returns `123.46`, and `rescale(123.456789, 4)` returns `123.4568`.

A negative `precision` rounds to the corresponding power of ten, so `rescale(123.456789, -1)` returns `120` and `rescale(123.456789, -2)` returns `100`.

If `val` already has no more than `precision` fractional digits then it is returned unchanged, so `rescale(123.456789, 8)` returns `123.456789`.
NaN, infinity, and zero are also returned unmodified, which means `rescale` never returns NaN for a finite input.
`precision` is saturated to the range of quantum exponents the format is able to encode.

NOTE: Unlike https://en.cppreference.com/w/cpp/numeric/math/trunc[trunc], `rescale` rounds rather than truncates. With the default `precision` of 0 it rounds to an integer using the active rounding mode, which by default is to nearest with ties to even, so `rescale(123.9)` returns `124`.
